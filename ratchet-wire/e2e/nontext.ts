 ...
 *
 * The first check earns its place in this repo immediately. Before it ran, this
 * lab had ONE border token — `--color-border`, `#30363d` dark and `#d0d7de`
 * light — doing two jobs with different requirements: dividing surfaces, where
 * no ratio is required, and drawing the ONLY boundary six kinds of control
 * have, where 3:1 is. Measured against the surface OUTSIDE each control, at the
 * colours this lab shipped:
 *
 *     `.btn-ghost`      1.42:1 dark / 1.45:1 light, over a transparent fill
 *                       (#guide-back, #mitm-demo-btn, #ooo-reset, #quiz-retry)
 *     `#reset-btn`      1.42:1 dark / 1.45:1 light, over a same-colour fill
 *     `.fs-point`       1.42:1 dark
 *     `#message-input`  1.55:1 dark / 1.36:1 light
 *     `.quiz-option`    1.55:1 dark / 1.36:1 light
 *     `#guide-reopen`   1.55:1 dark / 1.36:1 light
 *
 * There was no 1.4.11 check in this repo at all — not a weak one, none — so all
 * nine controls shipped unmeasured. The fix is a dedicated
 * `--color-control-border` (#6e7681 dark, #7d8590 light), applied to controls
 * only, leaving `--color-border` doing the divider job it is right for.
 *
 * A tenth and eleventh sat in the shared top bar — `a.cl-btn` and
 * `#cl-theme-toggle`, fill-less buttons whose 1px `color-mix(..., --accent 38%,
 * transparent)` border composited to 2.45:1 over the bar's #0b1512. Raised to
 * 50% (3.38:1) in this lab's own copy of the bar. Other labs in this fleet
 * baseline that pair instead; this one does not need to, so its baseline is
 * empty.
 *
 * The second check finds nothing here today: `style.css` declares no `content`
 * on any pseudo-element. It runs at every driven state anyway, which is what
 * makes that a measurement rather than a reading of the stylesheet.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A gradient is *sampled* at the sample point rather than judged at its worst
     * stop: linear by projecting onto the gradient line, radial by distance from
     * the centre over the farthest-corner radius. A non-gradient layer (`url()`,
     * `none`) is unmeasurable and paints nothing.
     *
     * This lab has exactly one gradient and it is a 1.4.11 case, which is why
     * the sampler is here rather than in `contrast.ts` alone: `.tooth.active` —
     * the highlighted "you are here" bar in the DH ratchet-step chart — is
     * `linear-gradient(135deg, var(--color-alice), var(--color-bob))`. Reading
     * its `background-color` would return `rgba(0, 0, 0, 0)` and report the bar
     * as having no fill at all. The radial branch is inert here; `body` paints a
     * flat `var(--color-bg)`.
     */
     ...
     *
     * All eight tab panels stay in the DOM whether or not they are selected, so
     * one pass judges every control in the lab at once — twenty-eight quiz
     * options, ten ratchet teeth, both comparison tables and every button on
     * every tab. Without the caches the pass re-reads the same computed styles
     * and rects thousands of times. Nothing mutates the DOM during the pass, so
