 ...
 *    an unknown backdrop, or a `color-mix()`. This lab uses `color-mix()` on
 *    exactly the surfaces that carry its verdicts: `.converge-row.match`, the
 *    green "both parties derived the same chain key" proof on the conversation
 *    tab, and `.fs-steal`, the red band marking the messages an attacker who
 *    stole a chain key can still read. The hero aside `.cl-hero-why` is a third,
 *    and the shared top bar's `--cl-ink` is a `color-mix(in srgb, …)` as well.
 *    axe resolves none of them; it files them under `incomplete`, so a
 *    violations-only gate never measured the two states this lab exists to show.
 ...
 *    rules and two of them are exempt rather than safe: `.btn:disabled` at 0.4
 *    and `.quiz-option:disabled` at 0.85 are inactive components, which WCAG
 *    1.4.3 exempts and which this walk skips for the same reason axe does. The
 *    third, `.cl-hero-sub { opacity: 0.85 }` on the subtitle beneath the H1, is
 *    live text and is measured through the composite rather than read off the
 *    declared value.
 *
 ...
 *    boxes are the two `.comparison-table` wrappers on the About tab
 *    (`overflow-x: auto`, and wide enough to scroll at 380px), the `.tab-content`
 *    panels (`overflow-y: auto`), `.wire-fields`, `.ooo-pending` and the
 *    `#mk-timeline` log. Content scrolled past one of those is not dimmed or
 *    partly drawn — it is absent, and asking what colour it sits on has no
 ...
 *    draws all four of its figures — the ratchet-tooth bar, the chain-key
 *    derivation strip, the message-key timeline and the forward-secrecy table —
 *    as CSS grids and flex rows of `<div>`s rather than in SVG, so the sibling
 *    walk finds nothing to do. Its only SVG is the shared top bar's two
 *    `aria-hidden` marks (a three-`<line>` hamburger and the GitHub `<path>`).
 ...
 * not one — nor is it one to axe's `color-contrast` rule. `style.css` currently
 * declares no `content` on any pseudo-element (its only `::before`/`::after`
 * selectors are the `box-sizing` reset and the two reduced-motion blocks), so
 * there is nothing here to measure today. `nontext.ts` checks the class anyway
 * at every driven state, which is what makes that a measurement rather than a
 * reading of the stylesheet — and what would catch the first decorative glyph
 * anyone adds.
 */
 ...
 * uses. Nothing here is short-lived: this lab's only animations are the 0.2s
 * `fadeIn` on `.tab-content` and the 0.25s `slideUp` on a new message bubble,
 * both of which the reduced-motion block this gate asserts collapses to
 * `0.01ms`, so every state the drive builds is fully settled before it is
 * measured (`settle()` in `gate.ts` waits for that to be true rather than
 * assuming it).
 *
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: the convergence proof,
     * the forward-secrecy steal band and the hero aside are all authored with
     * `color-mix()`, and Chromium reports that to
     ...
     * gradient whose worst stop spans its element, which stops being true the
     * moment a gradient runs the height of a document several screens tall. So
     ...
     *
     * This lab has exactly one gradient, and the sampler is what keeps it
     * honest: `.tooth.active` — the highlighted "you are here" bar in the DH
     * ratchet-step chart — is
     * `linear-gradient(135deg, var(--color-alice), var(--color-bob))`, two
     * different colours across one small box. The radial branch is inert here;
     * `body` paints a flat `var(--color-bg)`. It is kept because it is the same
     * code path, and because `nontext.ts` shares this sampler to judge that
     * tooth's fill against the chart behind it.
     */
     ...
     *
     * Every one of this lab's eight tab panels stays in the DOM whether or not
     * it is the selected one, so a single pass walks the whole catalogue at
     * once: the seven-question quiz with four options each, the two comparison
     * tables on the About tab, the per-message wire inspector, and the ten
     * `.tooth` bars — all of them re-walking the same ancestors up to `<body>`.
     ...
     * that is the two `.comparison-table` wrappers on the About tab, the
     * `.tab-content` panels, `.wire-fields`, `.ooo-pending`, `#mk-timeline` and
     * `.messages` — several of which genuinely scroll at 380px and none of which
     * do at 1280px, which is why the gate runs both widths.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // would apply the moment this lab redraws its ratchet chart in SVG rather
      // than as the `<div>` bars it uses today.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details>` at all; what it does ship is seven `.tab-content` panels
      // that are NOT the selected one, each held behind the `hidden` attribute,
      // plus `#guide-reopen`, `#mitm-result`, `#coach`, `#quiz-retry` and the
      // three `.sim-state` blocks on the Break-In Recovery tab. `boot()` probes
      // a live element to prove `hidden` really hides here, because `[hidden]`
      // is only (0,1,0) and any later class rule beats it.
      ...
      // `top:-3rem`, and the lab's own `.skip-link` parks at `top:-100%`.
      // Measuring either parked copy invents a failure for text that is not on
      // screen; the focused renderings are real states and the gate tabs to both
      // and scans them explicitly instead.
      ...
      //
      // This guard is LOAD-BEARING in this lab, not defensive. `style.css`
      // defines `.sr-only` with the classic `position: absolute; width: 1px;
      // height: 1px; clip: rect(0, 0, 0, 0)` recipe, and `index.html` applies it
      // fifteen times: the "Choose sender" `<legend>`, the message-input
      // `<label>`, the three `aria-describedby` targets on the Break-In Recovery
      // buttons, the two table `<caption>`s on the About tab, seven
      // "(opens in new tab)" spans, and `#sr-announcements`. Every one of them
      // keeps a 1x1 rect and passes `checkVisibility()`, so without this the
      // walk would report fifteen contrast failures for ink no one can see. It
     ...
     *
     * Every `aria-hidden` on this page was enumerated and checked rather than
     * assumed. There are seventeen, and they fall into exactly two shapes:
     *
     *  - A DECORATIVE LEADING GLYPH INSIDE A LABEL WHOSE TEXT IS EXPOSED. Every
     *    one of these is `<span aria-hidden="true">X </span>` immediately
     *    followed by the real words — `↺ Restart`, `✨ Send a sample message`,
     *    `🔓 Compromise Bob's Keys`, `📨 Alice Sends Message`,
     *    `🔒 Bob Receives (DH Ratchet)`, `⚠️ Attacker has compromised…`,
     *    `✓ Alice sent a message…`, `✓ Break-in recovery complete!`,
     *    `▸ Show guided tour`, `👋 A live, end-to-end-encrypted chat…`,
     *    `⚠️ Educational demo…` and `↺ Try again`. The glyph is hidden and the
     *    sentence beside it is not, so the ink that matters IS measured, by the
     *    span's own parent.
     *  - THE `→` ARROWS IN THE CHAIN-KEY DIAGRAM (`.arrow` and the leading
     *    `→ ` inside `.chain-label`). They are geometry, not content; the
     *    diagram's `role="img"` carries the sentence they stand for. Their ink
     *    is `--color-text-secondary` composited over the page, measured by hand
     *    at 9.06:1 dark and 6.00:1 light — clear of the 4.5:1 a 13px glyph is
     *    held to, either way.
     *
     * Nothing on this page hides a VALUE. No key fingerprint, no chain-key hex,
     * no message number, no verdict, no quiz feedback and no timeline entry is
     * inside an `aria-hidden` subtree — which was checked rather than assumed,
     * because this is the one place BOTH oracles stop looking.
     */
     ...
     *
     * This lab has no SVG of its own — every figure is built from `<div>`s in
     * `main.ts`. The guard stays
     ...
     *
     * THIS LAB IS THAT LAB. `style.css` sets `html, body { height: 100% }` and
     * paints the page from `body { background-color: var(--color-bg) }` — `html`
     * itself computes to `rgba(0, 0, 0, 0)`, measured, in both themes. So both
     * boxes are exactly one viewport tall while the About and Break-In Recovery
     * tabs run several viewports long, and the `<body>` fallback below is the
     * ONLY thing standing between this walk and reporting every element past the
     * first screen against a white page that does not exist. In dark that would
     * have inverted the sign of every ratio on the longest tabs.
     */
